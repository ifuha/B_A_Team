...
export default function StudentPdfPreview() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const result = await getJson<DetailResponse>(
        `/reports/student/${studentId}/detail`,
      );
      if (!result.ok) {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#7d90c2]">
      <AppHeader
        ...
        right={
          <a
            href={`/api/reports/student/${studentId}/pdf`}
            target="_blank"
            rel="noreferrer"
        ...
        {data && (
          <div className="mx-auto max-w-4xl rounded-md bg-white p-6 shadow">
            <div className="mb-4 flex flex-wrap gap-x-6 gap-y-1 border-b border-gray-300 pb-3 text-sm text-black">
              <span>氏名：{data.student.name}</span>
              <span>学籍番号：{data.student.studentNumber}</span>
              <span>専攻：{data.student.majorName}</span>
              <span>学年：{data.student.gradeLevel}年生</span>
            </div>
                ...
                {levels.map((level) => (
                  <div key={level}>
                    ...
                    <div className="space-y-2">
                      {data.gradeYears[level].map((entry, i) => {
                        const highlight =
